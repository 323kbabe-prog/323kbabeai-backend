...
function genderFromArtist(artist = "") {
  ...
  if (
    ["ariana", "sabrina", "doja", "rihanna", "beyonce", "taylor"].some((n) =>
      lower.includes(n)
    )
  )
    return "female";
  if (
    ["bieber", "tyler", "kendrick", "eminem", "drake"].some((n) =>
      lower.includes(n)
    )
  )
    return "male";
...
function chooseVoice(artist = "") {
  const lower = artist.toLowerCase();
  if (
    ["ariana", "sabrina", "doja", "rihanna", "taylor"].some((n) =>
      lower.includes(n)
    )
  )
    return "shimmer";
...
function stylizedPrompt(title, artist, styleKey = DEFAULT_STYLE) {
  const s =
    STYLE_PRESETS[styleKey] || STYLE_PRESETS["stan-photocard"];
  return [
    `create a high-impact, shareable cover image for the song "${cleanForPrompt(
      title
    )}" by ${cleanForPrompt(artist)}.`,
    ...
    "the performer should appear as a young " +
      genderFromArtist(artist) +
      " korean idol (gen-z style).",
    ...s.tags.map((t) => `• ${t}`),
  ].join(" ");
}
...
app.get("/api/trend", async (_req, res) => {
  try {
    ...
    if (nextPickCache) {
      ...
      nextPickCache = null;
      generateNextPick(); // pre-gen again
    } else {
      const pick = await nextNewestPick();
      const prompt = stylizedPrompt(pick.title, pick.artist);
      const imageUrl = await generateImageUrl(prompt);
