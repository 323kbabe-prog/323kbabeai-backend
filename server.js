...
// Optional fetch polyfill (only if not on Node 18/20)
...
// Simple rate limiting to protect APIs & costs
const trendLimiter = rateLimit({ windowMs: 60_000, max: 60, standardHeaders: true, legacyHeaders: false });
...
const lastKeys = [];
function pushKey(k, n=3){ lastKeys.push(k); while(lastKeys.length>n) lastKeys.shift(); }
function isRecent(k){ return lastKeys.includes(k); }
...
async function getSpotifyToken() {
  ...
  const resp = await withTimeout(fetch("https://accounts.spotify.com/api/token", {
    method: "POST",
    headers: {
...
async function generateImageUrl(prompt) {
  ...
  for (const model of models) {
    try {
      const out = await withTimeout(openai.images.generate({
        model, prompt,
        size: "1024x1024",
      ...
      console.error(`[images] ${model} failed:`, e?.response?.data?.error?.message || e?.message || String(e));
    }
  }
  return null;
}
...
app.get("/api/trend", async (req, res) => {
  ...
  const storefront = String((req.query.storefront||"us")).replace(/[^a-z-]/gi,"").slice(0,10).toLowerCase();
