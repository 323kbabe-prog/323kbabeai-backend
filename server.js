// server.js — Info Set v1.2 KV Mirror (safe, with rotating unique picks)
...
function dedupe(items) {
  ...
  return items.filter(x => {
    const key = `${(x.title||"").toLowerCase()}::${(x.artist||"").toLowerCase()}`;
...
async function searchCover({title,artist}){
  ...
  try {
    ...
    const j = await r.json();
    cover = j?.tracks?.items?.[0]?.album?.images?.[0]?.url || null;
  } catch {}
  return cover;
}
...
async function loadTrending({ market="US", storefront="us" } = {}){
  ...
  if (!items.length) items = [{ title:"Espresso", artist:"Sabrina Carpenter" }];

  trendingCache = { data: shuffle(dedupe(items)), expires: now + 60*1000 }; // refresh every 1 min
